...
| 결정자 | 사용자 |
| 결정일 | 2026-05-02 |
| 결정 사유 | (a)/(b)/(c) 셋 다 정직성 측면에서 모순이 없으나, (b)는 이번 발견(선언과 실측 괴리)을 박제만 하고 미루므로 다음 인스턴스까지 추가 학습 없음. (c)는 향후 실체화 경로를 닫아 "지휘자 모델"의 미래를 잠시 폐기. (a)는 8개 중 어느 것도 폐기하지 않으면서 기본 `up`에서 격리 — 첫 케이스가 요구하는 시점에 활성화하는 case-driven 경로를 보존. 정직성 시그널이 부분적으로 존재한다는 §2의 완화 요인이 (a)의 비용을 낮춘다 |

### 후속 PR (분리)

본 PR 머지 직후 다음 작업을 별도 PR로 진행:

1. `docker-compose.yml` 8 컨테이너에 `profiles: legacy-demo` 적용 — 기본 `docker-compose up`에서 모두 비활성. 명시적 opt-in 시에만 기동(`docker-compose --profile legacy-demo up`)
2. `services/langchain-api/`와 `services/n8n-workflows/`는 트리 그대로 보존. 첫 활성화 후보는 case #004의 두 번째 인스턴스 누적 시 — 그 시점에 langchain-api + chromadb만 격리 해제 후 Layer 3 컨텍스트 패커 코드화
3. `README.md`의 "현 상태" 표 + "로컬 실행 (현 데모 코드)" 섹션 갱신 — "현 데모 컨테이너는 기본 비활성. 첫 실체화 진입은 case #004 두 번째 인스턴스 시점"
4. `docs/INTEGRATIONS.md` §2.2 Antigravity 인터페이스: 가정 유지(=폐기 안 함, 톤 다운 안 함). §6의 case #009 행은 "(a) 격리 결정 — §2.2 가정 유지, 첫 실체화 진입은 별도 검증자 통합 케이스"로 갱신
5. `cases/README.md` 인덱스의 case #009 행을 ⏰(pending-decision)에서 🔨(decided-implement, 코드화 PR과 함께 ✅로 전환)로 갱신
